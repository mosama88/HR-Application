...
class CurrencyFaker extends Base
{
    ...
    {
        $available = array_filter(static::$cities, function ($city) {
            return !in_array($city['name'], static::$usedCities);
        });

        if (empty($available)) {

            static::$usedCities = [];  // Reset the used categories
            $available = static::$cities;  // Reset the available categories
        }

        $city = static::randomElement($available);
        static::$usedCities[] = $city['name'];
